Feature: Sequencer
  ...
  Scenario: Read miss handled by storing expected message
    Given sequencer "A" receives StartInstance with instance ID "0x1", period ID "2", sequence number "2", and xtrequest "1: [tx1], 2: [tx2]"
    When the execution engine simulates "tx1" and returns a read miss for message with source chain ID "2", destination chain ID "1", sender "0xabc", receiver "0xdef", session ID "0x123", label "MSG", and data "data"
    Then sequencer "A" store mailbox message with source chain ID "2", destination chain ID "1", sender "0xabc", receiver "0xdef", session ID "0x123", label "MSG", and data "data" as expected message for instance ID "0x1"
  ...
  Scenario: A received mailbox message that is expected is added to the inbox and triggers a new simulation
    Given sequencer "A" receives StartInstance with instance ID "0x1", period ID "2", sequence number "2", and xtrequest "1: [tx1], 2: [tx2]"
    And the execution engine simulates "tx1" and returns a read miss for message with source chain ID "2", destination chain ID "1", sender "0xabc", receiver "0xdef", session ID "0x123", label "MSG", and data "data"
    And sequencer "A" store mailbox message with source chain ID "2", destination chain ID "1", sender "0xabc", receiver "0xdef", session ID "0x123", label "MSG", and data "data" as expected message for instance ID "0x1"
  ...
  Scenario: Timeout after voting causes no action
    Given sequencer "A" receives StartInstance with instance ID "0x1", period ID "2", sequence number "2", and xtrequest "1: [tx1], 2: [tx2]"
    And sequencer "A" sent Vote to the shared publisher with instance ID "0x1", chain ID "1", and vote "_"
    When the timer for instance ID "0x1" expires
